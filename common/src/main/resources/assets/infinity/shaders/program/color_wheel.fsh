...
void main() {
    ...
    float time = 2*(IridTimeSec / 100.0f)*3.1415f;
    float iTime = 10*time + noiseScale2*noise(vec3(noiseScale1*screenCoords, 2*noiseScale2*time));
    ...
    float warpScale = 0.1 * IridProgress / 3;
    float edgeFactor = 1 - 2*min(min(texCoord.x, 1 - texCoord.x), min(texCoord.y, 1 - texCoord.y));
    float texOffsetX = noise(vec3(3*screenCoords, 5*(time + 10000))) - 0.5;
    float texOffsetY = noise(vec3(3*screenCoords, 5*(time - 10000))) - 0.5;
    ...
    OutColor = (Chroma * (1.8 + 0.1*IridLevel)) + Luma;
